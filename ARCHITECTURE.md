...
**Last updated:** 2026-07-02 · **Release:** v0.18.0
...
`AiClient` (`ai/client.rs`) wraps a single `reqwest::Client` (300s timeout, set for slow free OpenRouter models), the normalised reasoning `effort` string, and an internal `enum AiClientConfig` with one variant per provider. v0.17 cut the set to three native HTTP providers; v0.18 **restored the Claude CLI subscription path** as a fourth, so the set is now: three native HTTP backends + one subprocess backend (only the OpenAI-compatible proxy stays removed — its legacy config value aliases to Anthropic on load):
...
Both SSE readers share `SseLineBuf`, a byte-based line accumulator (unit-tested), so a multi-byte UTF-8 character split across network chunks can never drop data. The CLI envelope decode is unit-tested (`claude_cli_envelope_parses_usage`).
